...
namespace TCAPP.DataAccess.Mapping
{
    public class UserEntityTypeConfiguration : IEntityTypeConfiguration<User>
    {
        public void Configure(EntityTypeBuilder<User> builder)
        {
            ...
            builder.HasMany(x => x.UserContents).WithOne(x => x.User).HasForeignKey(x => x.IdUser).HasConstraintName("FK_UserContent_User");
            builder.HasMany(x => x.Collections).WithOne(x => x.User).HasForeignKey(x => x.IdUser).HasConstraintName("FK_Collection_User");
        }
    }
}
