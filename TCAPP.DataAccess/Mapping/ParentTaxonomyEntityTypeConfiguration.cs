...
namespace TCAPP.DataAccess.Mapping
{
    public class ParentTaxonomyEntityTypeConfiguration : IEntityTypeConfiguration<ParentTaxonomy>
    {
        public void Configure(EntityTypeBuilder<ParentTaxonomy> builder)
        {
            ...
            builder.HasOne(x => x.Taxonomy).WithMany(x => x.ParentTaxonomies).HasForeignKey(x => x.IdTaxonomy);
            builder.HasOne(x => x.Parent).WithMany(x => x.ParentTaxonomies).HasForeignKey(x => x.IdParentTaxonomy).HasConstraintName("FK_ParentTaxonomy_Taxonomy2");
        }
    }
}
